...
type OrbProps = {
  ...
  className?: string
  /** When set, fixes luminance inversion for dark UIs without a `dark` class on &lt;html&gt;. */
  uiInverted?: boolean
}
...
function Scene({
  ...
  useLayoutEffect(() => {
    if (uiInverted === undefined) return
    if (!circleRef.current) return
    circleRef.current.material.uniforms.uInverted.value = uiInverted ? 1 : 0
  }, [uiInverted])
  ...
  const uniforms = useMemo(() => {
    ...
    const invertedInit =
      uiInverted !== undefined ? (uiInverted ? 1 : 0) : isDark ? 1 : 0
    return {
      ...
      uInverted: new THREE.Uniform(invertedInit),
      uInputVolume: new THREE.Uniform(0),
      uOutputVolume: new THREE.Uniform(0),
      uOpacity: new THREE.Uniform(0),
    }
  }, [perlinNoiseTexture, offsets, uiInverted])
